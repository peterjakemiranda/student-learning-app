    ...
    <div v-else>
      ...
      <template v-if="quizzes.length">
        <q-item  class="q-pa-sm"  v-for="quiz in quizzes" :key="quiz.id">
          <template v-if="(isStudent && !quiz.archive) || isTeacher">
            ...
            <q-item-section>
              ...
              <q-item-label caption>
                <span v-if="quiz.archive">Status: <span class="text-warning">Archived</span></span>
                <span v-else-if="quiz.started">Status: <span class="text-green">Active</span> <br> Started at: {{quiz.started_date_formatted}}</span>
            ...
            <q-item-section side>
              <div class="row q-gutter-xs" v-if="isTeacher">
                ...
                <q-btn v-ripple v-if="!quiz.started && !quiz.archive" size="12px" color="primary" label="Start" @click="toggleStatus(quiz,1)" :loading="toggleLoading"/>
                <q-btn v-ripple v-else-if="!quiz.archive" size="12px" color="red" label="Stop" @click="toggleStatus(quiz,0)" :loading="toggleLoading"/>
              </div>
              <div class="row q-gutter-xs" v-else-if="quiz.started">
                <q-btn :disable="quiz?.answers.length > 0" v-ripple :to="`/courses/${course.id}/quizzes/${quiz?.id}/take`" color="primary" size="12px" label="Take" />
              </div>
            </q-item-section>
          </template>
